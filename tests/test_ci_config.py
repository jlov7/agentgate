...
def test_verify_strict_job_is_scheduled_weekly() -> None:
    workflow = _load_workflow()
    ...
    run_step = next(step for step in strict_job["steps"] if step["name"] == "Run verify strict")
    assert run_step["run"] == "make verify-strict"


def test_load_test_p95_budget_is_not_too_strict() -> None:
    workflow = _load_workflow()

    load_test_steps = workflow["jobs"]["load-test"]["steps"]
    ...
    assert run_load_test_step["continue-on-error"] == "${{ github.event_name == 'schedule' }}"
